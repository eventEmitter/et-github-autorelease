!function(){
	...
	module.exports = new Class({
		...
		, release: function(data, config) {
			...
			exec('cd ' + config.path + ' && git pull', function(err, stdout, stderr) {
				if (err) {
					...
					log('done');
					if (config.command) this.executeCommand(config.command, config);
					else if (config.service) this.restartService(config.service);
				}
			}.bind(this));
		}
		...
		, executeCommand: function(command, config) {
			exec('cd ' + config.path + ' && '+command, function(err, stdout, stderr) {
				if (err) {
					log(stdout, stderr, err);
				}
				else {
